...
{
    u64 zeros = 0;
    zeros += cc::count_trailing_zeros(v.d[0]);
    if constexpr (w > 1)
    {
        if (zeros < 64)
            return zeros;
        zeros += cc::count_trailing_zeros(v.d[1]);
    }
    if constexpr (w > 2)
    {
        if (zeros < (2 * 64))
            return zeros;
        zeros += cc::count_trailing_zeros(v.d[2]);
    }
    if constexpr (w > 3)
    {
        if (zeros < (3 * 64))
            return zeros;
        zeros += cc::count_trailing_zeros(v.d[3]);
    }
    return zeros;
}
...
{
    u64 ones = 0;
    ones += cc::count_trailing_zeros(~v.d[0]);
    if constexpr (w > 1)
    {
        if (ones < 64)
            return ones;
        ones += cc::count_trailing_zeros(~v.d[1]);
    }
    if constexpr (w > 2)
    {
        if (ones < (2 * 64))
            return ones;
        ones += cc::count_trailing_zeros(~v.d[2]);
    }
    if constexpr (w > 3)
    {
        if (ones < (3 * 64))
            return ones;
        ones += cc::count_trailing_zeros(~v.d[3]);
    }
    return ones;
}
