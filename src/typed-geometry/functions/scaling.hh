...
template <int D, class T>
TG_NODISCARD constexpr mat<D + 1, D + 1, T> scaling(T v)
{
    return mat<D + 1, D + 1, T>::diag(vec<D + 1, T>(vec<D, T>(v), T(1)));
}

template <class T>
TG_NODISCARD constexpr mat<3, 3, T> scaling(T sx, T sy)
{
    return mat<3, 3, T>::diag(vec<3, T>(sx, sy, T(1)));
}
template <class T>
TG_NODISCARD constexpr mat<4, 4, T> scaling(T sx, T sy, T sz)
{
