...
FUZZ_TEST("AreaRelations")(tg::rng& rng)
{
    ...
    CHECK(aAabb == approx(aBox)); // conversion from aabb to box does not affect the area
    ...
    CHECK(aSphere == approx(2.f * aHemisphereNoCaps)); // sphere == 2 * hemisphere (3D)
    CHECK(aHemisphere3 == approx(aHemisphereNoCaps + aHemisphereCaps)); // hemisphere = round half sphere + flat base
    ...
    CHECK(aCylinder == approx(aCylinderMantle + 2.f * aDiskIn3)); // cylinder == mantle + 2 caps
    CHECK(aCapsule == approx(aCylinderMantle + aSphere)); // capsule == mantle + 2 caps
    CHECK(aCapsule == approx(aCylinder + aHemisphereNoCaps)); // relation between capsule and cylinder
...
FUZZ_TEST("AreaObjects")(tg::rng& rng)
{
    ...
    // triangle
    {
        ...
        {
            CHECK(triangleArea == approx(area_of(triangle)));
        }
...

        {
            ...
            CHECK(equiTriangleArea * 2.0f == approx(area_of(aabb_of(equiTriangle))));
        }
    }
    {
        // 3D
    ...
    // cylinder
    {
        ...
        CHECK(area_of(c) == approx(h * 2 * r * tg::pi<tg::f32>.radians() + 2 * tg::pow2(r) * tg::pi<tg::f32>.radians()));
    }

    // disk
    {
        const auto d = tg::sphere2in3(pos30, r, n3);
        CHECK(area_of(d) == approx(tg::pi_scalar<tg::f32> * tg::pow2(r)));
    }

    // sphere
    {
        const auto s = tg::sphere3(pos30, r);
        CHECK(area_of(s) == approx(4 * tg::pi_scalar<tg::f32> * tg::pow2(r)));
    }
    ...
    // regular pyramid
    {
        const auto check_pyramid_area = [](const auto& py, auto slantHeight)
        {
            ...
            CHECK(area_of(py) == approx(perimeter * slantHeight / 2.f + baseArea));
            CHECK(area_of(boundary_no_caps_of(py)) == approx(perimeter * slantHeight / 2.f));
        };
